...
public class ScreenUtils implements Constants {
    ...
    public static Bitmap getRoundBitmap(Context ctx, int diameter, final Bitmap source, int resColorBg, int padding) {
        ...
        double scaleFactor = width >= height ? (double) (diameter - (padding * 2)) / Math.sqrt(Math.pow(width, 2) + Math.pow(width, 2)) : (double) (diameter - (padding * 2)) / Math.sqrt(Math.pow(height, 2) + Math.pow(height, 2));

        Bitmap scaledBitmap = Bitmap.createScaledBitmap(source, (int) (scaleFactor * width), (int) (scaleFactor * height), true);
